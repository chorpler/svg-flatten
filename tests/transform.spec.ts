...
describe('svg-flatten: transform function', function() {
    it('should apply translate transformation', function() {
        ...
        // test
        expect(SvgTransform.transform(source).toString()).to.be.equal(target.toString());
    });
    ...
    it('should work with complex paths', function() {
        ...
        // test
        expect(SvgTransform.transform(circle).toString()).to.be.equal('<path d="M93 200a17 17 0 1 0 34 0 17 17 0 1 0-34 0"/>');
        expect(SvgTransform.transform(ellipse).toString()).to.be.equal('<path d="M199 200a42 11 90 1 0 22 0 42 11 90 1 0-22 0"/>');
        expect(SvgTransform.transform(line).toString()).to.be.equal('<path d="M310 200L360 250"/>');
        expect(SvgTransform.transform(polygon).toString()).to.be.equal('<path d="M110 10L50 198 200 78 20 78 170 198z"/>');
        expect(SvgTransform.transform(polyline).toString()).to.be.equal('<path d="M410 150L460 250 410 300"/>');
        expect(SvgTransform.transform(rect).toString()).to.be.equal('<path d="M510 200L590 200 590 300 510 300z"/>');
        expect(SvgTransform.transform(shape).toString()).to.be.equal('<path d="M1432 416.1c-6.3-40.5 83.3-114 173-108 90.1 6 173.9 92 175 188 1 85.5-64.1 140.2-79 152-97.2 76.6-220.5 30.1-236 24-84.8-33.6-153-114.3-138-142 22.1-40.7 210.4 57.4 263-10 21.5-27.5 16-77-2-87-19.5-10.8-44.9 29.6-91 26C1468.8 456.8 1435.5 438.3 1432 416.1z"/>');
    });
});
